...
def train_xe(model, dataloader, text_field,gpt_optimizer,args):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        count = 0
        for it, (detections, captions) in enumerate(dataloader):
            if captions.max() == 63999:
                while captions.max() == 63999:
                    index = floor(captions.argmax()/captions.size()[1])
                    captions = torch.cat((captions[0:index],captions[index+1:]))
                    captions = torch.cat((captions[0:torch.cuda.device_count()-1],captions[(torch.cuda.device_count()-1)*2:]))
                    detections = torch.cat((detections[0:index],detections[index+1:]))
                    detections = torch.cat((detections[0:torch.cuda.device_count()-1],detections[(torch.cuda.device_count()-1)*2:]))
...
if __name__ == '__main__':
   ...
   # image_field2 = ImageDetectionsField(detections_path="./flicker.hdf5 ", max_detections=50, load_in_tmp=False)
    image_field_XM = ImagesField_noncoco(args.features_path)
    # Pipeline for text
    ...
    #dataset_val = CC3M(image_field_XM, text_field_XM, ann_root = "./annotations/CC3M_ar_coco_format_clean.json", img_root = "./CC3M_images_sample.h5")

    # Create the dataset
    train_dataset =  CC3M(image_field_XM, text_field, ann_root = args.annotation_folder, img_root = args.features_path)
    ...
    with wandb.init(mode="offline",project="VGPTAR",config=config):
        ...
        for e in range(start_epoch, start_epoch + 100):
            ...
            train_loss = train_xe(model, dataloader_train, text_field,gpt_optimizer,args)
            ...
            'use_rl': use_rl,
        }, 'saved_models/%s_loss_CC3M.pth' % str(train_loss))

            # if switch_to_rl and not best:
            #     print(" now we are resuming!!!!")
            #     data = torch.load('saved_models/%s_best.pth' % args.exp_name,map_location='cpu')
